    ...
    <script>
      $(function () {
          ...
          $("input[name='username']").blur(function () {
              ...
              $.get(
                  "${pageContext.request.contextPath}/user/findUpdateUserName.do",
          ...
          $("#saveUrl").click(function () {
              $("#saveForm").attr("action","${pageContext.request.contextPath}/user/updateUser.do");
          ...
          $("select[name='pids'] option").each(function () {
              ...
              <c:forEach items="${user.postList}" var="post">
                if ("${post.pid}" == optionVal){
        ...
        <div class="ItemBlockBorder">
            <div class="ItemBlock">
                <table cellpadding="0" cellspacing="0" class="mainForm">
                        ...
                        <td>
                            <select name="departmentId">
                                ...
                                <c:forEach items="${departmentList}" var="department">
                                    <option value="${department.did}" <c:if test="${department.did == user.departmentId}">selected</c:if> >${department.dname}</option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <td>登录名</td>
                        <td><input type="text" name="username" value="${user.username}" class="InputStyle"/> *(登录名要唯一)<span id="usernameMsg"></span></td>
                    </tr>
                    <tr>
                        <td>性别</td>
                        <td>
                            <input type="radio" value="${user.sex}" name="sex" <c:if test="${user.sex=='男'}">checked</c:if> id="male" /><label for="male">男</label>
                            <input type="radio" value="${user.sex}" name="sex"  <c:if test="${user.sex=='女'}">checked</c:if> id="female" /><label for="female">女</label>
                        </td>
                    </tr>
                    <tr>
                        <td>E-mail</td>
                        <td><input type="email"  value="${user.email}" name="email" class="InputStyle"/></td>
                    </tr>
                </table>
            </div>
        </div>
